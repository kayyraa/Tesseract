...
for (const Conversation of ConversationsData) {
    ...
    ConversationClone.addEventListener("mousedown", () => ConversationClone.style.backgroundColor = "color-mix(in srgb, var(--TextColorTertiary), rgb(255, 89, 89) 50%)");
    ...
    ConversationClone.ClickAndHold((_) => {
        ...
        Tip.innerHTML = Mobile
            ? "Click and hold on the conversation again to remove it."
            : "Right click on the conversation to remove it.";
    ...
    ConversationClone.addEventListener("mousedown", async (_) => {
        if (_.button !== 1) return;
        const OtherUsername = Conversation.Users.filter(User => User !== `${Local.Name}#${Local.Tag}`)[0];
        localStorage.setItem("LastConversationTesseract", Conversation.id);

        const Update = async () => {
            Array.from(Chat.children).forEach(Node => {
                if ([ "InputContainer", "AttachmentContainer", "Patch" ].includes(Node.getAttribute("class"))) return;
                Node.remove();
            });
            ...
            for (let Index = 0; Index < Doc.Messages.length; Index++) {
                ...
                if (Local.Name === Message.Author) {
                        ...
                        const RemoveMessage = async () => {
                            ...
                            await new FireStorage("Conversations").UpdateDocument(
                                Conversation.id,
                                {
                        ...
                        MessageInstance.ClickAndHold(() => {
                            ...
                            setTimeout(() => Tip.style.opacity = 1, 250);
                            Tip.innerHTML = Mobile 
                                ? "Click and hold on the message again to remove your message." 
                                : "Press [MOUSE RIGHT BUTTON] to remove your message.";
                        
                            if (Mobile) {
                                let MobileListenerAdded = false;
                                if (!MobileListenerAdded) {
                                    MessageInstance.ClickAndHold(async () => await RemoveMessage());
                                    MobileListenerAdded = true;
                                }
                            } else {
                                const KeyListener = async (_) => {
                                    if (_.button !== 2) return;
                                    await RemoveMessage();
                ...
                Chat.appendChild(MessageInstance);
            }
