...
for (const Conversation of ConversationsData) {
    ...
    if (IsGroup) {
        ...
        ConversationClone.setAttribute("Original", OtherNames);
        ConversationClone.setAttribute("Display", OtherNames);
        ...
        ConversationClone.setAttribute("Original", OtherUsername);
        ConversationClone.setAttribute("Display", OtherUsername.split("#")[0]);
    ...
    ConversationClone.addEventListener("mouseenter", () => {
        const Span = ConversationClone.querySelector("span");
        Span.innerHTML = ConversationClone.getAttribute("Original");

        const SpanRect = Span.getBoundingClientRect();

        const Padding = 68;
        const MinLeft = Padding / 2 - SpanRect.width - 16;
        const MaxLeft = SpanRect.width + Padding - 16;

        if (SpanRect.width > 244 - Padding - 16) {
            ConversationClone.setAttribute("Oversize", "true");
            Span.style.position = "absolute";
            Span.style.left = MinLeft + "px";
            ...
            Span.Animation = Animation;
        }
    });
    ConversationClone.addEventListener("mouseleave", () => {
        const Span = ConversationClone.querySelector("span");
        if (Span.Animation) Span.Animation.cancel();
        Span.innerHTML = ConversationClone.getAttribute("Display");
    ...
    ConversationClone.addEventListener("click", async (_) => {
        ...
        const Update = async () => {
            ...
            MessageInput.oninput = () => SendButton.style.opacity = MessageInput.value ? 1 : 0;
...
AttachmentInput.addEventListener("change", async (_) => {
    ...
    const AttachmentContainer = Chat.querySelector(":scope > .AttachmentContainer");
    ...
    ImageInstance.addEventListener("mouseenter", () => {
        Tip.setAttribute("style", "");
        Tip.innerHTML = Mobile ? "Click on the attachment to remove it" : "Click with the left mouse button to remove it";
    });
    ImageInstance.addEventListener("mouseleave", () => {
        Tip.style.opacity = 0;
        setTimeout(() => Tip.style.display = "none", 250);
    });
});
